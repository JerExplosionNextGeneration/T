...
class ImageryCache {
    ...
    private func getImagy(nsKey: NSString) -> UIImage? {
        return imgCache.object(forKey: nsKey)
    }
    private func addImagy( imigi: UIImage, nsKey: NSString) {
        imgCache.setObject(imigi, forKey: nsKey)
    }
    
    public func fetchImigi(url: URL, callBack: @escaping (_ imigi: UIImage?) -> ()) {
        let aImg = getImagy(nsKey: url.absoluteString as NSString)
        ...
        if aImg != nil {
            ...
            session.dataTask(with: url) { [weak self] (data, response, erro) in
                ...
                self?.addImagy(imigi: img, nsKey: url.absoluteString as NSString)
